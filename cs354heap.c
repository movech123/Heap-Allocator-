...
static size_t get_block_size(BlockHeader *header) {
    ...
    size_t size = header->size_status & SIZE_BITMASK;
...
static void set_block_abit(BlockHeader *header, int abit) {
     if (abit == ABIT_USED) {
        // Set the A-bit (mark the block as used)
        header->size_status |= A_BITMASK;
    } else if (abit == ABIT_FREE) {
        // Clear the A-bit (mark the block as free)
        header->size_status &= ~A_BITMASK;
}
}
/*
...
static void set_block_pbit(BlockHeader *header, int pbit) {
     header->size_status = (header->size_status & ~P_BITMASK) | pbit;
}
...
static void *get_block_payload(BlockHeader *header) {
    if (header == NULL){
        ...
        void* payload_ptr = (char*)header + sizeof(BlockHeader);
        return payload_ptr;
    }
}
...
static BlockHeader *get_next_adjacent_block(BlockHeader *header) {
    if (header == NULL) {
        // Handle the case when the input header is NULL.
        return NULL;
    }
    
    BlockHeader *next_header = (BlockHeader*)((char*)header + get_block_size(header));
    char *heap_end = (char *)heap_start + heap_size;
    if ((void *)next_header < (void *)heap_start || (void *)next_header >= (void *)heap_end) {
        return NULL;
    }

    if (get_block_size(header) >= heap_size){
    return NULL;
    }
...
static BlockHeader *best_fit_select_block(size_t size) {
    ...
    while (current != NULL && current->size_status !=0){
    if (get_block_abit(current) == ABIT_USED){ //skip used blocks
      current = get_next_adjacent_block(current);
      continue;
      }
        size_t curr_block_size = get_block_size(current);
        if (curr_block_size >= size) {
            size_t wastage = curr_block_size - size;
            if(wastage > 0) { // incase current block is smaller then we skip
            // Check if this block minimizes wastage
                if (wastage < smallest_waste) {
                    best_fit_block = current;
                    smallest_waste = wastage;
                }
            }else if (wastage == 0){ // exact size found 
                 set_block_abit(current,ABIT_USED);
...
static void make_block_free(BlockHeader *header) {
    ...
    set_block_pbit(header, PBIT_USED);
     BlockFooter *footer = (BlockFooter *)((char *)header + get_block_size(header) - sizeof(BlockFooter));
    footer->size = get_block_size(header);
...
static BlockHeader *split_block(BlockHeader *header, size_t desired_size) {
   size_t original_size = get_block_size(header);
    size_t remaining_size = original_size - desired_size;
    if (remaining_size < MIN_BLOCK_SIZE){
        return NULL;
    }
    // Calculate the pointer to the new free block's header
    BlockHeader *new_free_block = (BlockHeader *)((char *)header + desired_size);
    set_block_header(new_free_block, remaining_size, ABIT_FREE, PBIT_USED);
    BlockFooter *new_free_block_footer = (BlockFooter *)((char *)new_free_block + remaining_size-sizeof(BlockFooter));

    // Update the footer of the new free block
    new_free_block_footer->size = remaining_size;
 ...
 *   size, increase it to the min block s
 * ize. Round up to a multiple of
...
void* balloc(size_t size) {
    ...
    // allocate block size and check if its smaller than min size 
    size_t block_size = size;
    if (block_size < MIN_BLOCK_SIZE){
        block_size = MIN_BLOCK_SIZE;
    }
    // Round up the block size to a multiple of BLOCK_ALIGNMENT
    block_size = round_up_block_size(block_size);
    // Get the best fit block header
    BlockHeader *address = best_fit_select_block(block_size);

    
    // Check and split if large enough 
    if(address == NULL){
      return NULL;
    }
    
    if (get_block_size(address) == block_size){ //best fit case just payload pointer 
            if (get_next_adjacent_block(address) != NULL){
            set_block_pbit(get_next_adjacent_block(address), PBIT_USED);
            return get_block_payload(address);
            }
            return get_block_payload(address);
    }
    if (get_block_size(address)  > MIN_BLOCK_SIZE){  
        split_block(address,block_size);
        return get_block_payload(address);
    }
    
    return NULL; // block not big enough found 
}
...
int bfree(void *ptr) {
    ...
    // Check if ptr is not aligned properly
